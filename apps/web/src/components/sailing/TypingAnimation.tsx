...
export default function TypingAnimation() {
  ...
  useEffect(() => {
    ...
    if (isDeleting) {
      ...
      if (displayedText === currentLine) {
        ...
        const typingSpeed = currentLineIndex === 3 ? 85 : 70;
  ...
  return (
    <div className="min-h-[120px] flex items-center justify-center">
      <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight text-center text-slate-900">
        {displayedText}
        <span className={`inline-block w-1 h-12 md:h-14 lg:h-16 ml-1 align-middle ${showCursor ? "opacity-100" : "opacity-0"} transition-opacity duration-100 bg-slate-900`} />
      </h1>
    </div>
  );
}
